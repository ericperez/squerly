...
/**
  *
  * Squerly - Transformation class to calculate max/max/mean values across series in a 2D array
  * 
  * 
  ...
  * 
  */
class Transform_Minmaxmean implements Transform_Interface {


/**
  *
  * Find the min/max/mean value across all series of a two-dimensional data array
  * 
  ...
  */
  public static function run(array $data, array $fields = array()) {
    ...
    foreach($data as $row) {
      ...
      $output[] = array($y_col => $y_val, 'MAX' => max($vals), 'MIN' => min($vals), 'MEAN' => array_sum($vals) / sizeof($vals));
    }
    return $output;
  }
