...
class Pivot
{
    ...
    public function fetch($fetchType=null, $add_row_counter = true)
    {
        $tmp = $tmpCount = array();
        //TODO: Handle columns not being set
        $split  = isset($this->_column[0]) ? $this->_column[0] : null;
        ...
        foreach ($this->_recordset as $reg) {
            switch (count($this->_pivotOn)) {
                case 1:
                    ...
                    if(is_null($this->_columnValues)) { continue; }
        ...
        return $this->_buildOutput($tmp, $fetchType, $tmpCount, $add_row_counter);
    }
    ...
    const _ID = '_id';
    private function _buildOutput($tmp, $fetchType, $tmpCount, $add_row_counter = true)
    {
        $out = array();
        $cont = 0;
        $fullTotal  = array();
        switch (count($this->_pivotOn)) {
            case 1:
                ...
                foreach ($tmp as $p0 => $p0Values) {
                    ...
                    if($add_row_counter) { $_out[self::_ID] = ++$cont; }
                    if ($this->_typeMark) {
                    ...
                    foreach (array_keys($this->_splits) as $split) {
                        $cols = isset($p0Values[$split]) ? $p0Values[$split] : null;
                        ...
                        foreach (array_keys($this->_splits[$split]) as $col) {
                            ...
                            foreach ($this->_columnValues as $_k) {
                                ...
                                if ($this->_lineTotal) {
                                    if(isset($_lineTotal[$k])) { $_lineTotal[$k] += $value; }
                                }
                                if ($this->_fullTotal) {
                                    if(isset($fullTotal[$split][$col][$k])) { $fullTotal[$split][$col][$k] += $value; }
                                }
                            }
                        }
                    }
                    if ($this->_lineTotal) {
                        foreach ($this->_columnValues as $_k) {
                            ...
                            $value = ($_k instanceof Pivot_Callback) ? 
                                $_k->cbk($_lineTotal) : (isset($_lineTotal[$k]) ? $_lineTotal[$k] : null);
            ...
            case 2:
                foreach ($tmp as $p0 => $p0Values) {
                    ...
                    foreach ($p0Values as $p1 => $p1Values) {
                        ...
                        foreach (array_keys($this->_splits) as $split) {
                            $cols = isset($p1Values[$split]) ? $p1Values[$split] : null;
        ...
        if ($this->_fullTotal) {
            ...
            if ($this->_lineTotal) {
                foreach ($this->_columnValues as $_k) {
                    $k = ($_k instanceof Pivot_Callback || $_k instanceof Pivot_Count) ? $_k->getKey() : $_k;
                    $value = ($_k instanceof Pivot_Callback) ? $_k->cbk($_lineTotal) : (isset($_lineTotal[$k]) ? $_lineTotal[$k] : null);
                    $_out[self::TOTAL . "_{$k}"] = $value;
                }
            }
            $out[] = $_out;
