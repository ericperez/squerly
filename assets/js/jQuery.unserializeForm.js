  ...
  var methods = {
    ...
    var settings = $.extend( {
	  ...
	  return this.each(function() {
          ...
          methods._pushValue(serialized_values, properties[0].replace(/\+/g, " "), decodeURIComponent(properties[1].replace(/\+/g, " ")));
  			}
  		});
  		...
  		for(var key in _values) {
  			var el = $(this).add("input,select,textarea").find("[name=\"" + decodeURIComponent(key) + "\"]");

        if(typeof(_values[key]) != "string") {
          // select tags using 'multiple' will be arrays here (reports "object")
          // We cannot do the simple decodeURIComponent() because it will flatten the array.
          // Instead, decodeURIComponent each item individually
          var _value = new Array();
          $.each(_values[key], function(i, v) {
            _value.push(decodeURIComponent(v));
          })
        } else {
  			  var _value = decodeURIComponent(_values[key]);
        }
  			...
  			if(_callback == undefined) {
  				...
  				var result = _callback.call(this, decodeURIComponent(key), _value, el);
